...
| E | Bayesian nonparametric ZNE with credible intervals | **Implemented (calibration partial)** | `estimators/bayesian.py` (tanh GP + marginal likelihood opt), `experiments/09_bayesian_coverage.py` | Coverage well-calibrated at low noise; under-covers at high noise with fixed hyperparameters; optimization helps but not fully validated | Medium |
| F | Sequential design (choose next scale) | **Prototype** | `estimators/bayesian.py::design_next_scale`, `experiments/10_sequential_design.py` | Greedy one-step only; candidate domain must be specified carefully (λ≥1 for ZNE-relevant); no global optimality | Medium |
| G | Operator/locality-aware extrapolation | **Prototype** | `theory/locality.py`, `estimators/locality_aware.py`, `experiments/11_locality_aware_proxy.py` | Heuristic proxy only; current mapping over-regularizes (negative result in exp 11); no commutator bounds; synthetic support-size scaling | Low (v0.9+ for calibration/theory) |
| H | Lower bounds / structured escape hatches | **Structured taxonomy** | `theory/structured_escape_hatches.py`, `docs/structured_lower_bound_taxonomy.md`, `paper/structured_escape_hatches.md` | Taxonomy only; no proven lower/upper bounds; no minimax results; classification is heuristic | Low (rigorous theory deferred) |
